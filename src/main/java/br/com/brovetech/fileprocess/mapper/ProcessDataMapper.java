...
public class ProcessDataMapper {
    ...
    public static String toFileOutput(ProcessedDataDTO processedDataDTO){
        return String.format("Quantidade de clientes: %d", processedDataDTO.getNumberOfCustomers()).concat(System.lineSeparator())
                ...
                .concat(String.format("Id venda mais cara: %d", Optional.ofNullable(processedDataDTO.getMostExpensiveSale().getId()).orElse(ZERO))).concat(System.lineSeparator())
                .concat(String.format("Pior vendedor: %s", Optional.ofNullable(processedDataDTO.getWorstSalesman().getName()).orElse(EMPTY)));
    }
}
